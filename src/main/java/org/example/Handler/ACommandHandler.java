...
public class ACommandHandler implements  Runnable{
    ...
    void handleAUCallTruck(UpsAmazon.AUCallTruck auCallTruck){
        ...
        System.out.println("store to uogopickup" + seqUtils.getValue() + pickUpBuilder.build().toString());
        for (long i : uGoPickupMap.keySet()){
            System.out.println("ugopickup storing key is " + i);
        }
        //database
        List<UpsAmazon.AUProduct> aProductList = auCallTruck.getThingsList();
        for (UpsAmazon.AUProduct aProduct : aProductList){
            if (packagesMapper.getPackageById((int) aProduct.getId()) == null){
                packagesMapper.insertPackage(new Packages((int) aProduct.getId(), truck.getTruckId(),
                    ...
                    aProduct.getDestX(), aProduct.getDestY()));
            }
        }
    ...
    void handleAUAcks(long ack){
        System.out.println("ack from amz" + ack);
    ...
    void handleAUTruckGoDeliver(UpsAmazon.AUTruckGoDeliver auTruckGoDeliver){
        ...
        System.out.println("store to uogodeliver" + seqUtils.getValue() + uGoDeliverBuilder.build().toString());
        for (long i : uGoDeliverMap.keySet()){
            System.out.println("ugodeliver storing key is " + i);
        }
        uaACKSet.add(auTruckGoDeliver.getSeqnum());
    }

    void handleUserInfo(UpsAmazon.AURequestSendUserInfo auRequestSendUserInfo){
        System.out.println(auRequestSendUserInfo.toString());
        uaACKSet.add(auRequestSendUserInfo.getSeqnum());
        if (userMapper.getUserById(Integer.toString(auRequestSendUserInfo.getUserid()))== null){
    ...
    public void run(){
        ...
        while (true){
            try {
                ...
                System.out.println("read from amz");
                System.out.println(auCommands.toString());
                //AUCallTruck
