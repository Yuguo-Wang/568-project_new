...
public class WCommandHandler implements  Runnable{
    ...
    void handleAcks(long ack){
        System.out.println("get ack from world " + ack);
        System.out.println("uogopickmap size " + uGoPickupMap.size());
        ...
        if (uGoPickupMap.containsKey(ack)){
            ...
            System.out.println("uogopickmap size (after) " + uGoPickupMap.size());
        }
    ...
    void handleUFinished(WorldUps.UFinished uFinished){
        ...
        ///===================change======================================================//
        List<Packages> packages = packagesMapper.getPackagesByTruckId(uFinished.getTruckid());
        List<Long> packageIds = new ArrayList<>();
        for (Packages packages1: packages){
            packageIds.add((long)packages1.getPackageId());
        }
        builder.setSeqnum(seqUtils.getValue()).setTruckid(uFinished.getTruckid()).setX(uFinished.getX()).setY(uFinished.getY()).addAllPackageid(packageIds);
        if (! truckMapper.getTruckById(uFinished.getTruckid()).getStatus().equals(ConstUtils.TRUCK_IDLE_STATUS)) {
...
//        }
        ...
        while (true){
            try {
                ...
                System.out.println("Command from world");
                System.out.println(uResponses.toString());
                if (uErrList.size() > 0){
